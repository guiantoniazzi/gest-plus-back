			...
			this.router.get(
				...
				async (req: Request, res: Response): Promise<any> => {
					try {
						...
						const perfisAcesso =
							await this.perfisAcessoService.getPerfisComFuncoes();
						return res.status(200).json(perfisAcesso);
					} catch (error) {
						return res
							.status(500)
							.json({ message: "Erro ao buscar perfis de acesso" });
					}
				}
			);
			...
			this.router.post(
				...
				async (req: Request, res: Response): Promise<any> => {
					try {
						...
						const { nomePerfil, ativo, idsFuncoesSistema } = req.body;
						...
						const novoPerfil = await this.perfisAcessoService.cadastrarPerfil(
							{
								nomePerfil,
								ativo,
								idsFuncoesSistema,
